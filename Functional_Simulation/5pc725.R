...
Y_5725 <- Y_rand + rep(f_true, times=N) # simulated data (fixed + random)
save(Y_5725, file = "Y_5725.Rdata")

lam_5pc725 <- matrix(0, N, ngroups) # store smoothing parameter
fef_5pc725 <- matrix(0,n,ngroups) # store estimated fixed-effect
pcs_5pc725 <- array(data = 0, c(n,npc,ngroups)) # store principal components 
CG_fun_5pc725 <- array(data = 0, c(n,n,ngroups)) # store genetic covariance function
CE_fun_5pc725 <- array(data = 0, c(n,n,ngroups)) # store environmental covariance function
res_5pc725 <- rep(0,ngroups) # store estimated residual
sfit_5pc725 <- rep(NA, ngroups) # whether the cov is singular
...
for (k in 1:ngroups){
  ...
  df_sm <- data.frame(id = id, y= Y_5725[,k], 
                      phi1 = b1, 
                      phi2 = b2, 
                      phi3 = b3, 
                      phi4 = b4,
  ...
  CG_fun_5pc725[,,k] <- CG_fun# estimated gen cov function
  CE_fun_5pc725[,,k] <- CE_fun# estimated env cov function
  
  ### Extract estimated residual variance
  res_5pc725[k] <- sigma(ft)^2
  
  ### Check singular fit
  sfit_5pc725[k] <- isSingular(ft)
  
  save(lam_5pc725, file = "lam_5pc725.Rdata")
  save(pcs_5pc725, file = "pcs_5pc725.Rdata")
  save(fef_5pc725, file = "fef_5pc725.Rdata")
  save(CG_fun_5pc725, file = "CG_fun_5pc725.Rdata")
  save(CE_fun_5pc725, file = "CE_fun_5pc725.Rdata")
  save(res_5pc725, file = "res_5pc725.Rdata")
  save(sfit_5pc725, file = "sfit_5pc725.Rdata")
}
