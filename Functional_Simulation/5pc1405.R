...
Y_51405 <- Y_rand + rep(f_true, times=N) # simulated data (fixed + random)
save(Y_51405, file = "Y_51405.Rdata")

lam_5pc1405 <- matrix(0, N, ngroups) # store smoothing parameter
fef_5pc1405 <- matrix(0,n,ngroups) # store estimated fixed-effect
pcs_5pc1405 <- array(data = 0, c(n,npc,ngroups)) # store principal components 
CG_fun_5pc1405 <- array(data = 0, c(n,n,ngroups)) # store genetic covariance function
CE_fun_5pc1405 <- array(data = 0, c(n,n,ngroups)) # store environmental covariance function
res_5pc1405 <- rep(0,ngroups) # store estimated residual
sfit_5pc1405 <- rep(NA, ngroups) # whether the cov is singular
...
for (k in 1:ngroups){
  ...
  df_sm <- data.frame(id = id, y= Y_51405[,k], 
                      phi1 = b1, 
                      phi2 = b2, 
                      phi3 = b3, 
                      phi4 = b4,
  ...
  CG_fun_5pc1405[,,k] <- CG_fun# estimated gen cov function
  CE_fun_5pc1405[,,k] <- CE_fun# estimated env cov function
  
  ### Extract estimated residual variance
  res_5pc1405[k] <- sigma(ft)^2
  
  ### Check singular fit
  sfit_5pc1405[k] <- isSingular(ft)
  
  save(lam_5pc1405, file = "lam_5pc1405.Rdata")
  save(pcs_5pc1405, file = "pcs_5pc1405.Rdata")
  save(fef_5pc1405, file = "fef_5pc1405.Rdata")
  save(CG_fun_5pc1405, file = "CG_fun_5pc1405.Rdata")
  save(CE_fun_5pc1405, file = "CE_fun_5pc1405.Rdata")
  save(res_5pc1405, file = "res_5pc1405.Rdata")
  save(sfit_5pc1405, file = "sfit_5pc1405.Rdata")
}
