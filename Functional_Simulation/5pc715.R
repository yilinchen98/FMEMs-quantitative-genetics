...
Y_5715 <- Y_rand + rep(f_true, times=N) # simulated data (fixed + random)
save(Y_5715, file = "Y_5715.Rdata")

lam_5pc715 <- matrix(0, N, ngroups) # store smoothing parameter
fef_5pc715 <- matrix(0,n,ngroups) # store estimated fixed-effect
pcs_5pc715 <- array(data = 0, c(n,npc,ngroups)) # store principal components 
CG_fun_5pc715 <- array(data = 0, c(n,n,ngroups)) # store genetic covariance function
CE_fun_5pc715 <- array(data = 0, c(n,n,ngroups)) # store environmental covariance function
res_5pc715 <- rep(0,ngroups) # store estimated residual
sfit_5pc715 <- rep(NA, ngroups) # whether the cov is singular
...
for (k in 1:ngroups){
  ...
  df_sm <- data.frame(id = id, y= Y_5715[,k], 
                      phi1 = b1, 
                      phi2 = b2, 
                      phi3 = b3, 
                      phi4 = b4,
  ...
  ft <-  fit_genetic_fmm(fform, df_sm, A, 5)
  
  ## Extract fixed-effect
  beta <- fixef(ft)
  fef_5pc715[,k] <- pcs %*% beta
  ...
  CG_fun <- pcs[,1:4] %*% CG %*% t(pcs[,1:4]) # estimated gen cov function
  CE_fun <- pcs[,1:4] %*% CE %*% t(pcs[,1:4]) # estimated env cov function
  
  CG_fun_5pc715[,,k] <- CG_fun# estimated gen cov function
  CE_fun_5pc715[,,k] <- CE_fun# estimated env cov function
  
  ### Extract estimated residual variance
  res_5pc715[k] <- sigma(ft)^2
  
  ### Check singular fit
  sfit_5pc715[k] <- isSingular(ft)
  
  save(lam_5pc715, file = "lam_5pc715.Rdata")
  save(pcs_5pc715, file = "pcs_5pc715.Rdata")
  save(fef_5pc715, file = "fef_5pc715.Rdata")
  save(CG_fun_5pc715, file = "CG_fun_5pc715.Rdata")
  save(CE_fun_5pc715, file = "CE_fun_5pc715.Rdata")
  save(res_5pc715, file = "res_5pc715.Rdata")
  save(sfit_5pc715, file = "sfit_5pc715.Rdata")
}
