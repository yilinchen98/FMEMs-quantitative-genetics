...
Xi <- as(cbind(rep(pcs43[,1], times = N),rep(pcs43[,2], times = N),rep(pcs43[,3], times = N),rep(pcs43[,4], times = N)), Class = "sparseMatrix" )# raw random effect matrix
...
CG_para <- as(kronecker(A, CG_hat_43), "sparseMatrix") # reparameterised genetic covariance 
CE_para <- as(kronecker(I_N, CE_hat_43), "sparseMatrix") # reparameterised environmental covariance 

mu_hat <- rep(0, dim(CG_para)[1])
mu_res_hat <- rep(0, N*n)
res_cov_hat <- as(sig2_hat_43 * I_n, "sparseMatrix") 
...
Y_est43_1 <- matrix(0, n*N,n_iter)

lam_sap43_1 <- matrix(0, N, n_iter) # store the smoothing parameter
pcs_sap43_1 <- array(data = 0, c(n,npc,n_iter)) # store the PCs
fef_sap43_1 <- matrix(0,n,n_iter) # store estimated fixed-effect
CG_fun_sap43_1 <- array(data = 0, c(n,n,n_iter)) # 3d matrix store genetic covariance function of 50 iterations
CE_fun_sap43_1 <- array(data = 0, c(n,n,n_iter)) # 3d matrix store environmental covariance function of 50 iterations
res_sap43_1 <- rep(0,n_iter) # store estimated residual
sfit_sap43_1_strict <- rep(NA, n_iter)
sfit_sap43_1 <- rep(NA, n_iter)
...
for (k in 1:n_iter){
  ...
  ### simulated data
  Yk <-  rep(fef_hat_43, times = N) + Zi %*% t(alpha_hat) + Zi %*% t(gamma_hat) + t(res_hat) 
  Y_est43_1[,k] <- Yk@x
  save(Y_est43_1, file = "bs_sample_51050_43_1.Rdata")
  ...
  lam_sap43_1[,k] <- lamk
  save(lam_sap43_1, file = "lam_sap43_1.Rdata")
  
  ## FPCA
  fpcaobjk <- prcomp(t(yk_hat), center = TRUE, retx = TRUE, rank. = npc)
  pcs <- fpcaobjk$rotation
  pcs_sap43_1[,,k] <- pcs
  save(pcs_sap43_1, file = "pcs_sap43_1.Rdata")
  ...
  fform <- y ~ -1 + df_sm$phi1 + df_sm$phi2 + df_sm$phi3 + df_sm$phi4 + df_sm$phi5 + df_sm$phi6 +
    (-1 + df_sm$phi1 + df_sm$phi2 + df_sm$phi3  + df_sm$phi4 | df_sm$id) + 
    (-1 + df_sm$phi1 + df_sm$phi2 + df_sm$phi3  + df_sm$phi4 | df_sm$id)
  ...
  beta <- fixef(ft)
  fef_sap43_1[,k] <- pcs %*% beta
  
  ### Extract covariance
  vc <- VarCorr(ft)
  CG <- vc[["df_sm.id"]] # genetic covariance
  CE <- vc[["df_sm.id.1"]] # environmental covariance
  res_sap43_1[k] <- sigma(ft)^2
  
  CG_fun_sap43_1[,,k] <- pcs[,1:4] %*% CG %*% t(pcs[,1:4]) # estimated gen cov function
  CE_fun_sap43_1[,,k] <- pcs[,1:4] %*% CE %*% t(pcs[,1:4]) # estimated env cov function
  
  sfit_sap43_1_strict[k] <- isSingular(ft)
  sfit_sap43_1[k] <- isSingular(ft, tol = 1e-5)
  
  save(fef_sap43_1, file = "fef_sap43_1.Rdata")
  save(CG_fun_sap43_1, file = "CG_fun_sap43_1.Rdata")
  save(CE_fun_sap43_1, file = "CE_fun_sap43_1.Rdata")
  save(sfit_sap43_1, file = "sfit_sap43_1.Rdata")
  save(sfit_sap43_1_strict, file = "sfit_sap43_1_strict.Rdata")
  save(res_sap43_1, file = "res_sap43_1.Rdata")
}
